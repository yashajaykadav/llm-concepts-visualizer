...
export default function TokenizerModule() {
  ...
  useEffect(() => {
    ...
    if (mockTokenDatabase[cleaned]) {
      tokenData = mockTokenDatabase[cleaned];
    } else {
      tokenData = generateFallbackTokens(inputText);
    }
  ...
  const getTokenColor = (token, index) => {
    if (hoveredTokenIndex === index) return token.color.replace('bg-', 'bg-').replace('/10', '/30');
    return token.color;
  };
          ...
          <div className="flex items-start gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-[200px]">
              ...
              <p className="text-slate-300 mt-3 leading-relaxed max-w-3xl">
                LLMs don't understand raw text directly—they split it into smaller units called{' '}
                <strong className="text-indigo-400">tokens</strong> and convert them into numerical IDs.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-950/50 backdrop-blur-sm border border-slate-800/50 rounded-2xl p-6 hover:border-slate-700/50 transition-colors">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                <button
                  onClick={() => handleExampleClick("learning generative ai is fascinating")}
                  className="
                ...
                <button
                  onClick={() => handleExampleClick("tokenizers break text into pieces")}
                  className="
                    px-3 py-1.5 rounded-lg text-xs
                    bg-slate-800/50 text-slate-300 border border-slate-700/50
                    hover:bg-slate-700/50 hover:border-slate-600
                ...
                <button
                  onClick={() => handleExampleClick("the quick brown fox jumps over the lazy dog")}
                  className="
                    px-3 py-1.5 rounded-lg text-xs
                    bg-slate-800/50 text-slate-300 border border-slate-700/50
                    hover:bg-slate-700/50 hover:border-slate-600
            ...
            <div className="bg-slate-950/50 backdrop-blur-sm border border-slate-800/50 rounded-2xl p-6 hover:border-slate-700/50 transition-colors space-y-6">
              <div>
                ...
                <div className="min-h-[100px] bg-slate-900/50 rounded-xl border border-slate-800/50 p-5 flex flex-wrap items-center gap-2 transition-all">
                  {tokens.length > 0 ? (
                    tokens.map((token, index) => (
                      <span
                        ...
                        className={`
                          px-3 py-2 rounded-lg border font-mono text-sm font-semibold
                          transition-all duration-200 cursor-pointer
                          ${token.color}
                          ${hoveredTokenIndex === index 
                            ? "scale-110 ring-2 ring-white/30 shadow-xl z-10" 
                            : "hover:scale-105"}
                        `}
                      >
              ...
              <div>
                ...
                <div className="bg-slate-900/50 border border-slate-800/50 rounded-xl p-5 font-mono flex flex-wrap items-center gap-1 transition-all min-h-[60px]">
                  ...
                  {tokens.length === 0 && (
                    <span className="text-slate-500 text-sm font-sans">No tokens</span>
                  )}
                  <span className="text-indigo-400 font-bold">]</span>
                </div>
              </div>
              ...
              <div className="space-y-4 flex-1">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-slate-300">
                        Words can split into <strong className="text-purple-400">sub-tokens</strong> for better handling of new words.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-800/50">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-slate-300">
                        This allows models to handle <strong className="text-green-400">unknown words</strong> by breaking them down.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-800/50">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-slate-300">
                        Every token gets a <strong className="text-blue-400">unique numerical ID</strong> for the model to process.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 bg-amber-950/20 border border-amber-900/30 rounded-xl p-3">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs text-amber-300/80">
                    <strong className="text-amber-200">Tip:</strong> Hover over tokens to see their IDs and highlight relationships.
                  </p>
                </div>
              </div>
            </div>
