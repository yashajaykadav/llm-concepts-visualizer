...
import { BookOpen, Binary, Database, Cpu, Lock, ChevronRight, Sparkles, Menu, X } from 'lucide-react';
...
export default function Layout() {
  ...
  // Locked modules will be available in future
  const menuItems = [
    { 
      id: 'tokenizer', 
  ...
  const getModuleStatus = (status) => {
    switch(status) {
      ...
      case 'upcoming':
        return { bg: 'bg-amber-500/20', text: 'text-amber-400', label: 'Coming Soon' };
      case 'locked':
        return { bg: 'bg-slate-700/30', text: 'text-slate-500', label: 'Locked' };
      default:
        return { bg: 'bg-slate-700/30', text: 'text-slate-500', label: 'Locked' };
    }
  };
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center space-y-4">
              <div className="p-6 bg-slate-800/50 rounded-2xl border border-slate-700/50">
                ...
                <p className="text-slate-500">This module is currently in development</p>
              </div>
            </div>
          </div>
        );
    ...
    <div className="flex h-screen bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 text-slate-100 font-sans overflow-hidden">
      ...
      <aside 
        className={`
          ...
          w-72 lg:w-80
          bg-slate-950/95 lg:bg-slate-950
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isActive && !isLocked
                    ? 'bg-gradient-to-r from-indigo-600/30 to-indigo-600/10 text-white shadow-lg shadow-indigo-600/10 border border-indigo-500/30'
                    : isLocked
                    ? 'text-slate-600 cursor-not-allowed opacity-60'
                    : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200 hover:border-slate-700/50'
                  }
                  ${!isLocked && !isActive ? 'border border-transparent' : ''}
                `}
              >
                <div className={`
                ...
                <div className={`
                  ...
                  ${isActive ? 'ring-1 ring-indigo-400/30' : ''}
                `}>
                  {status.label}
                </div>
          ...
          {/* Progress Section */}
          <div className="mt-8 pt-6 border-t border-slate-800/50 px-3">
            <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-800/50">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-slate-400">Progress</span>
                <span className="text-xs text-indigo-400 font-semibold">1/3</span>
              </div>
              <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-indigo-500 to-cyan-400 rounded-full transition-all duration-500" style={{ width: '33.33%' }} />
              </div>
        ...
        <div className="p-4 border-t border-slate-800/50">
          <div className="flex items-center gap-3 px-3 py-2 bg-slate-900/50 rounded-xl border border-slate-800/50">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-slate-200 truncate">Learning Mode</p>
              <p className="text-[10px] text-slate-500">Interactive Session</p>
            </div>
            ...
            <div className="lg:hidden flex items-center justify-between mb-4 pt-12">
              <div>
                <h1 className="text-xl font-bold text-white">
                  {menuItems.find(item => item.id === activeModule)?.name || 'Module'}
                </h1>
